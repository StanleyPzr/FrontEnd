<template>
  <div class="container" style="margin: 0 auto; margin-top: 20px;">
    <b-button variant="primary" @click="ObtenerRegistroCambio">Obtener Registros de Cambios</b-button>

    <b-table v-if="registrosCambios.length > 0" :items="registrosCambios" :fields="fields" class="mt-3 center-table table-bordered">
...
export default {
  ...
  methods: {
    async ObtenerRegistroCambio() {
      try {
        ...
        if (response) {
          ...
          this.mostrarDetalles = new Array(response.length).fill(false); 
        } else {
    ...
    toggleDetallesCambio(index) {
      this.$set(this.mostrarDetalles, index, !this.mostrarDetalles[index]);
    },
    formatoFecha(value) {
      if (!value) return '';
