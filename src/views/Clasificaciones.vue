<template>     
    <div>
      <b-form-checkbox v-model="mostrarFilasSinDatos">Mostrar filas sin datos</b-form-checkbox>
      <b-table
        :items="clasificacionesFiltradas"
        :fields="camposTabla"
        striped
        hover
        responsive="sm"
      >                 
      </b-table>
      <b-pagination
        v-model="paginaActual"
        :total-rows="totalclasificaciones"
        :per-page="10"
        aria-controls="mi-tabla"
      ></b-pagination>
    </div>
  </template>
  
  <script>
  import Estado from '@/helpers/Estado.service.js';
  
  export default {
    data() {
      return {
        clasificaciones: [],
        camposTabla: [
          { key: 'id', label: 'Id' },
          { key: 'descripcion', label: 'Descripción' },
                  
        ],     
        mostrarFilasSinDatos: false,
        paginaActual: 1,
        totalclasificaciones: 0,
      };
    },
    computed: {
        clasificacionesFiltradas() {
        // Filtrar jornadas según el estado del checkbox
        let clasificacionesFiltradas = this.mostrarFilasSinDatos ? this.clasificaciones.filter(clasificacion => {
          // Verificar si alguna de las propiedades está vacía o es nula
          return Object.values(clasificacion).some(valor => valor === '' || valor === null);
        }) : this.clasificaciones;
  
        // Aplicar paginación
        const inicio = (this.paginaActual - 1) * 10;
        const fin = this.paginaActual * 10;
        return clasificacionesFiltradas.slice(inicio, fin);
      },
    },
    mounted() {
      this.obtenerClasificaciones();
    },
    methods: {
      async obtenerClasificaciones() {
        const data = await Estado.ObtenerJuegosComunes();
        if (data && data.clasificaciones) {
          console.log('Clasificaciones obtenidas:', data.clasificaciones);
          console.log('all data obtenidas:', data);         
          this.clasificaciones = data.clasificaciones;
          this.totalclasificaciones = data.clasificaciones.length;
        }
      },
      debeAgregarDato(item, columna) {
        return item[columna] === '' || item[columna] === null;
      },
      agregarDato(item, columna) {      
        console.log('Agregar dato para:', columna, 'de', item);      
        item[columna] = '01234567890123'; 
      },
    },
  };
  </script>
  